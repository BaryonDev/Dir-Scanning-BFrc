...
async def main():
    ...
    if found_directories:
        # Menggunakan CuPy untuk menghitung panjang direktori yang ditemukan
        data_lengths = cp.array([len(dir) for dir in found_directories])
        logging.info(f"Lengths of found directories (on GPU): {data_lengths}")
        
        # Contoh pengolahan lebih lanjut di GPU
        mean_length = cp.mean(data_lengths)
        logging.info(f"Mean length of found directories (on GPU): {mean_length}")
